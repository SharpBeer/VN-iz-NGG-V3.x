...
hook OnPlayerKeyStateChange(playerid, newkeys, oldkeys) {
	...
	if(newkeys & KEY_YES) {
		...
		format(szMiscArray, sizeof(szMiscArray), "Khu vuc: %d, %d, %d, %d, %d, %d, %d, %d, %d, %d,",
		...
		if(szAreas[0] != INVALID_STREAMER_ID) {
			for(new i; i < MAX_GROUP_LOCKERS; ++i) {
				...
				if(szAreas[0] == arrGroupLockers[PlayerInfo[playerid][pMember]][i][g_iLockerAreaID]) {
					cmd_locker(playerid, "");
					format(szMiscArray, sizeof(szMiscArray), "AreaID: %d, Khoa Area ID: %d", szAreas[0], arrGroupLockers[PlayerInfo[playerid][pMember]][i][g_iLockerAreaID]);
...
hook OnPlayerEnterDynamicArea(playerid, areaid)	{
	...
	format(szMiscArray, sizeof(szMiscArray), "Truy cap khu vuc: %d", areaid);
	SendClientMessage(playerid, COLOR_YELLOW, szMiscArray);
}

hook OnPlayerLeaveDynamicArea(playerid, areaid)	{

	format(szMiscArray, sizeof(szMiscArray), "Roi khoi khu vuc : %d", areaid);
	SendClientMessage(playerid, COLOR_YELLOW, szMiscArray);
}